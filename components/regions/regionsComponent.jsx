    ...
    useEffect(() => {
        if (filterStr) {
            let cloned = JSON.parse(JSON.stringify(regionGroups)).map(group => {
                ...
                if (group.name.toLowerCase().includes(filterStr.toLowerCase())) {
                    group.regions.push({
                        address: '',
                        email: '',
                        employees: [],
            ...
            let clonedRegionGroups = JSON.parse(JSON.stringify(regionGroups)).map(group => {
                group.regions = group.regions.filter(region => {
                    return region.regionName.toLowerCase().includes(filterStr.toLowerCase());
                });
                return group;
        ...
        <>
            ...
            <div className="c-container mt-20 md:mt-[120px] xl:mt-[150px] pb-[120px] xl:pb-[200px]">
                ...
                ">{filteredData.map(group => group.regions && group.regions.length > 0 && <div key={group.name}>
                    ...
                    sm:text-[16px]
                    ">{group.name}</Link>
                        {group.regions.filter(region => region.group !== null).map(region => <div key={region.regionSlug} className="mt-4 text-[16px] leading-[150%]
                    md:text-[18px]
                    ">
